...
export default function BibleDictionaryExplorer() {
  ...
  const [mobileView, setMobileView] = useState<"results" | "entry">("results");
    ...
    <section className="space-y-6 md:space-y-8">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 md:p-8">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-sm leading-7 text-zinc-300 md:text-lg">
      ...
      <div className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
        <aside
          className={`rounded-3xl border border-white/10 bg-white/5 p-4 md:p-5 ${
            mobileView === "entry" ? "hidden md:block" : "block"
          }`}
        >
          ...
          {results.length === 0 ? (
            ...
            <div className="space-y-3 md:max-h-[680px] md:overflow-y-auto md:pr-1">
              {results.map((entry) => {
        ...
        <div
          className={`rounded-3xl border border-white/10 bg-white/5 p-5 md:p-8 ${
            mobileView === "results" ? "hidden md:block" : "block"
          }`}
        >
          {!selectedEntry ? (
